...
export function ClosetClient() {
  ...
  const [aiStatus, setAiStatus] = useState<"unknown" | "ready" | "starting">(
    "unknown",
  );
  ...
  useEffect(() => {
    fetch("/api/ai/warmup")
      .then((r) => r.json())
      .then((d) =>
        setAiStatus(d?.data?.status === "ready" ? "ready" : "starting"),
      )
  ...
  const handleDelete = useCallback(
    async (id: string) => {
      ...
      if (result.error) {
        ...
        toast({
          title: "Item removed",
          description: "Deleted from your wardrobe.",
        });
        refresh();
      }
    },
      ...
      <div className="space-y-8 animate-fade-in">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-3 flex-shrink-0 flex-wrap justify-end">
            ...
            {/* Bulk classify */}
            {total > 0 && <BulkClassifyButton items={items} onDone={refresh} />}
        ...
        <FilterBar
          category={category}
          onCategory={(v) => setCategory(v as any)}
          search={search}
          onSearch={setSearch}
          total={total}
        />
