...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      "fixed inset-0 z-50 bg-black/70 backdrop-blur-sm data-[state=open]:animate-fade-in-slow data-[state=closed]:opacity-0 transition-opacity",
      className
    )}
    {...props}
  />
...
>(({ className, children, showClose = true, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      {showClose && (
        <DialogClose className="absolute right-4 top-4 p-1.5 rounded-lg text-foreground-faint hover:text-foreground hover:bg-surface-2 transition-colors focus:outline-none focus:ring-1 focus:ring-accent">
          <X className="h-4 w-4" />
